...
class WriteImageVirtual(object):
    ...
    image_fmt_arguments = property(
        get_image_fmt_arguments, set_image_fmt_arguments)
...
class WriteImageExtended(WriteImageVirtual):
    ...
    def __init__(self):
        ...
        if not imported_imageio:
            raise UnsupportedWriterException(
                " %s , imageio library is not installed: " % sys._getframe().f_code.co_name)
